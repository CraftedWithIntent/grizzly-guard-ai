...
If you discover a security vulnerability in Grizzly, please report it responsibly to:
...
### What Grizzly Protects Against

Grizzly is designed as a **deterministic firewall layer** for LLM safety. It protects against:
...
### What Grizzly Does NOT Protect Against

Grizzly is **NOT** a replacement for:
...
- **Data privacy compliance** — Grizzly can help with PII masking, but doesn't replace encryption or compliance frameworks
- **Complete prompt injection defense** — Use Grizzly + application-level validation together

### Best Practices

When using Grizzly in production:

1. **Combine with application-level validation** — Don't rely solely on Grizzly
...
7. **Review Grizzly updates** — Subscribe to security advisories

## Known Limitations

### Deterministic vs. Adaptive Defense

Grizzly uses **deterministic rules** (regex, semantic checks, schema validation) for speed (<5ms latency). This means:

- ✅ **Fast:** Runs locally without cloud calls
- ✅ **Predictable:** Same input → same output
- ⚠️ **Limited:** Can't adapt to novel attack patterns like an LLM-based guardrail

For cutting-edge threat detection, combine Grizzly with periodic LLM-based reviews.

### Regex-Based Detection

Grizzly includes regex patterns for common prompt injection attacks. Attackers may:
- Use encoding (base64, HTML entities)
- Obfuscate payloads
- Use natural language variations

Mitigation: Combine Grizzly's regex patterns with semantic similarity checks and periodic security audits.
...
Grizzly depends on well-maintained libraries:
...
Grizzly includes comprehensive security tests:
...
Grizzly **does not store or transmit data** by default. All processing happens in-process or in-memory. If you use the proxy server, review your deployment configuration for data retention policies.

### PII Handling

Grizzly can redact PII before it's logged or transmitted downstream. However:
...
If you deploy Grizzly in front of a cloud LLM API (OpenAI, Anthropic, etc.):
- Data flows: Your app → Grizzly → Cloud API
- Grizzly **does not log** requests to cloud APIs (configurable)
